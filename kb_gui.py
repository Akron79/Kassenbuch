...
import kb_export_all as export
# import kb_web_statistic as web
...
class main_window(tk.Tk):
   def __init__(self, ver, conn, tabel_name):
      ...
      verwaltmenu = tk.Menu(self.menu)
      self.menu.add_cascade(label="Verwaltung", menu=verwaltmenu)
      verwaltmenu.add_command(label="Datenbank Export", command=export.alles_exportieren('koschis.db'))
      # verwaltmenu.add_command(label="Statistik Webseite", command=web.externer_starten)
   ...
   def AddTopFrame(self):
      ...
      labelKasse = ttk.Label(master=self.upper_frame, style='KB.TLabel', text='Kassenbestand')
      ...
      kassenbestand = round(db.sqli.get_endbestand(self.connection,vor_Monat, vor_Jahr, 0),2) + round(db.sqli.get_monatssumme(self.connection, str(aktuellerMonat).zfill(2), aktuellesJahr,0),2)
