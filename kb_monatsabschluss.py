...
class main_window(tk.Tk):
  ...
  def AddInnerFrame(self):
    ...
    naechstes_jahr = aktuelles_jahr + 1

    jahr_options = [ int(vorjahr), int(aktuelles_jahr), int(naechstes_jahr)]
  ...
  def button_clicked(self):
    ...
    try:
        ...
        n_Anfangsbestand = round(db.sqli.get_endbestand(self.connection, vor_Monat, vor_Jahr, 1), 2)
        showinfo(title='Information', message=f"Anfangsbestand: {n_Anfangsbestand}")
        
        n_Endbestand = round(db.sqli.get_monatssumme(self.connection, n_Monat, n_Jahr, 1) + n_Anfangsbestand, 2)
        ...
        sql = f'''INSERT INTO monatsabschluss ( Jahr, Monat, Anfangsbestand, Endbestand, Gueltig, Zeitstempel) 
                 values(?, ?, ?, ?, ?, ?)'''
        data = [n_Jahr, n_Monat, n_Anfangsbestand, n_Endbestand, b_gueltig, datetime.datetime.now()]
        # showinfo(title='Information', message=f"SQL: {sql}")
        # for d in data:
        #    print(d)
        print('create_datev')
        self.connection.create_datev(n_Monat, n_Jahr)
        print('insert Datensatz für Abschluss')
        db.sqli.insert_or_update_values(self.connection, data, sql)
